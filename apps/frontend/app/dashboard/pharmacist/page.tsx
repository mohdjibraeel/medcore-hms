  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white p-4">
      <div className="flex items-start justify-between">
        <div>
          <div className="text-sm font-medium text-zinc-900">{item.medicineName}</div>
          <div className="text-sm text-zinc-500">
            {item.dosage}{item.dosageUnit} · {item.frequency} · {item.durationDays} days
          </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <Button
          size="sm"
          disabled={dispense.isPending || quantity < 1 || quantity > item.remaining}
...
function InventoryTable() {
  ...
  if (isLoading) return <p className="text-sm text-zinc-500">Loading inventory...</p>;
  ...
  return (
    <div className="space-y-2">
      {medicines?.map((med) => {
        ...
        const hasExpiredBatch = med.batches.some((b) => new Date(b.expiryDate) < new Date());
        ...
        return (
          ...
          >
            <div>
              <div className="text-sm font-medium text-zinc-900">{med.name}</div>
              <div className="text-sm text-zinc-500">
                {med.form} · {med.batches.length} batch(es)
                {hasExpiredBatch && <span className="ml-2 text-red-600">· contains expired stock</span>}
              </div>
            </div>
            <span
...
export default function PharmacistDashboardPage() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-lg font-semibold text-zinc-900">Pending Prescriptions</h1>
      <p className="mt-1 text-sm text-zinc-500">Prescription items still waiting to be dispensed.</p>

      <div className="mt-4 space-y-3">
        {isLoading && <p className="text-sm text-zinc-500">Loading...</p>}
        {isError && <p className="text-sm text-red-600">Couldn&apos;t load pending prescriptions.</p>}
      ...
      <p className="mt-1 text-sm text-zinc-500">Current medicine stock across all batches.</p>
      <div className="mt-4">
        <InventoryTable />
      </div>
    </div>
